...
def create_agents(P,rng):
	...
	from agents.duggins_agent_og import agent
	agentdict={}
...
def network_agents(agentdict):
	...
	for i in agentdict.values():
		for j in agentdict.values():
...
def plot_opinion_trajectory(dataframe,P):
	...
	sns.lineplot(x="time", y="opinion", data=dataframe, hue="agent", ax=ax1, legend=False, alpha=0.5)
	sns.lineplot(x="time", y="expressed", data=dataframe, hue="agent", ax=ax2, legend=False, alpha=0.5)
	# sns.lineplot(time="time",value="opinion",data=dataframe,unit="agent",ax=ax1) #err_style="unit_traces")
	# sns.lineplot(time="time",value="expressed",data=dataframe,unit="agent",ax=ax2) #err_style="unit_traces")
...
def main():
	...
	P=import_params('./opinion_dynamics/duggins/parameters.txt')
	...
	for t in np.arange(1,P['t_sim']+1):
		...
		order = np.array(list(agentdict.keys()))
	...
	# jsd_dataframe = pd.read_pickle('jsd.pkl')
